# Introduction to Using the Shell on Greene

This tutorial is an introduction to using the high-performance computing resources on Greene.  It is intended to give users a good introduction and overview of the tools available and how to use them effectively.
...
:::note[Overview]
Questions:
-   Why would I be interested in High Performance Computing (HPC) on Greene?
...
In all these cases, what is needed is access to more computers than can be used at the same time. Luckily, large a scale computing system — shared computing resources with lots of computers — is available at NYU and it is named Greene. Greene has more central processing units (CPUs), CPUs that operate at higher speeds, more memory, more storage, and faster connections with other computer systems. This kind of system is frequently referred to as a “cluster”, “supercomputer” or an “high performance computing” (HPC) system. In this lesson, we will usually use the terminology HPC and HPC cluster.
