...
```bash
[user@laptop ~]$ rsync -avP amdahl.tar.gz NYUNetID@greene.hpc.nyu.edu:
```
The options are:
...
```bash
[user@laptop ~]$ rsync -avP amdahl NYUNetID@greene.hpc.nyu.edu:~/
```
...
```bash
[user@laptop ~]$ rsync amdahl.tar.gz NYUNetID@greene.hpc.nyu.edu:
```
...
[user@laptop ~]$ rsync --port=768 amdahl.tar.gz NYUNetID@greene.hpc.nyu.edu:
```
:::note
