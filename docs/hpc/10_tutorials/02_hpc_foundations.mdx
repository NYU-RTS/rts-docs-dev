...
```mermaid
architecture-beta
    service greene(server)[Greene]
    service home(disk)[home]
    service scratch(disk)[scratch]
    service archive(disk)[archive]
    service vast(disk)[vast]
    service rps(disk)[research project space]
    service login(server)[login node]
...
| /vast      | /vast/**Net_ID**/    | High I/O Storage | $VAST

You will find more details about these filesystems at [Greene Storage Types page](../12_spec_sheet.mdx).
